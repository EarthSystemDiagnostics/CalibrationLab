...
Start with **`HANDOVER.md`**; this file is the short version plus the rules.
...
## Rules for changes
- **Never change the output file formats without updating `DATA_FORMATS.md`.**
...
- **Tests must stay hardware-free**: `python3 test_bath.py`,
  `python3 test_bisynch.py` (fake serial + simulator). Run both before
...
| Question | File |
|---|---|
| How do I take this over / run it? | `HANDOVER.md` |
| How do I operate it, what do the config keys mean? | `README.md` |
| What exactly is in the output files (for R)? | `DATA_FORMATS.md` |
| How do I talk to the bath, which commands exist? | `HANDOVER.md` §5, `bisynch.py` header |
| How would we fix the overshoot properly? | `PID_TUNING.md` |
